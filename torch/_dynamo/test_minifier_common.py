...
"""
    ...
    def _maybe_subprocess_run(self, args, *, isolate, cwd=None):
        if not isolate:
            ...
            dynamo_config = copy.copy(torch._dynamo.config)
            inductor_config = copy.copy(torch._inductor.config)
            try:
                stderr = io.StringIO()
                ...
                object.__setattr__(torch._dynamo, "config", dynamo_config)
                object.__setattr__(torch._inductor, "config", inductor_config)
